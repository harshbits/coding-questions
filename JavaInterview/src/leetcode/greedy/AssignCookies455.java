...
public class AssignCookies455 {
	...
	public static void main(String[] args) {
		int[] g = { 1, 2, 3 };
		int[]  s = {1, 1};
		var ans = new AssignCookies455().findContentChildren(g, s);
	...
	public int findContentChildren(int[] g, int[] s) {
		...
		// sort greedy factor
		Arrays.sort(g);
		// sort size of the cookie
		Arrays.sort(s);
	
		// Max cookies
		int max = 0, index = s.length - 1;
		for (int i = g.length - 1; i >= 0; i--) {
			
			if (index < 0) {
				return max;
			}
			// if size of the cookie is greater than the greedy factor
			if (s[index] >= g[i]) {
